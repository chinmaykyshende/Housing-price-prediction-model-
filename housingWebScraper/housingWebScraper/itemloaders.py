...
class TruliaItemsLoader(ItemLoader):
    ...
    def parse_property(self, response):
        ...
        if extract_with_css(response, '#propertySummary .addressContainer h1 div[data-role="address"]::text') is not None:
            ...
            loader.add_css(
                'address', '#propertySummary .addressContainer h1 div[data-role="address"]::text')

            city_state_zip = extract_with_css(
                response, '#propertySummary .addressContainer h1 span[data-role="cityState"]::text')
            # Extracts city, state and zip from strings like "Caldwell, NJ 07430" or "New Haven, CT 98930"
            res = re.findall(
                r"(?P<city>[A-Za-z\s]*),\s+(?P<state>[A-Z]*)\s+(?P<zip_code>[0-9]*)$", city_state_zip)
            if res:
            ...
            loader.add_css(
                'property_type', 'div[data-auto-test-id="home-detail"] div[data-auto-test-id="home-details-overview"] > .mbm div:first-of-type ul:first-of-type li:first-of-type::text')

            # Populate tax fields
            loader.add_css(
                'tax_year', 'div[data-auto-test-id="home-detail"] > div:nth-last-of-type(1) > .mbm > :nth-child(1) :nth-child(2)::text')
            loader.add_css(
                'tax_amount', 'div[data-auto-test-id="home-detail"] > div:nth-last-of-type(1) > .mbm > :nth-child(2) :nth-child(2)::text')
