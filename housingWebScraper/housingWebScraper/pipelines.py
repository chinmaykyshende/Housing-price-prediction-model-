...
class MultiCSVItemPipeline(object):
    ...
    def spider_opened(self, spider):
        self.files = dict([(name, open(settings.get(
            'CSV_OUTPUT_FOLDER') + name + '.csv', 'w+b')) for name in self.SaveTypes])
        self.exporters = dict([(name, CsvOptionRespectingItemExporter(
            self.files[name])) for name in self.SaveTypes])
